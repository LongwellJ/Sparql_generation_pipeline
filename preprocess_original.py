...
initial_path = f'../data/qald-9-plus/qald_9_plus_{dataset_type}_dbpedia_en.json'
final_path = f'../data/qald-9-plus/qald_9_plus_{dataset_type}_dbpedia_en_triples.json'
new_samples = []
prefixes = {}
with open(initial_path, 'r') as f:
  samples = json.load(f)
  for idx in trange(len(samples)):
    ...
    for answer in sample['answers']:
      if type(answer) != bool:
        assert len(answer.keys()) == 1 and len(answer.values()) == 1
        var = next(iter(answer.keys()))
        ans = next(iter(answer.values()))['value']
        if not var in answers.keys():
          answers[var] = [ans]
        else:
          answers[var].append(ans)
    sample['new_answers'] = answers
    ...
    sample['new_query'] = simplify_query(q, prefixes, exceptions)
